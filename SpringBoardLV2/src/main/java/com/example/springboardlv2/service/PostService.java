...
import com.example.springboardlv2.entity.User;
import com.example.springboardlv2.jwt.JwtUtil;
import com.example.springboardlv2.repository.PostRepository;
import com.example.springboardlv2.repository.UserRepository;
    ...
    public PostResponseDto createPost(PostRequestDto postRequestDto, HttpServletRequest request) { // 토큰이 확인되면 생성되는 구문을 만들자
        ...
        if (token != null) {
            if (jwtUtil.validateToken(token)) {
                claims = jwtUtil.getUserInfoFromToken(token); //사용자 정보 가져오기 (토큰을 쓰는 주된 이유) 임마 이거 조심해라 싸없새 등판 직전이다
            } else {
...
//        return postRepository.findAllByOrderByCreatedAtDesc();
    ...
    public PostResponseDto updatePost(Long id, PostRequestDto postRequestDto, HttpServletRequest request) {
        ...
        // 오늘 배운거 TIL 작성 하자 이것도 싸없새 되기 직전이다! 객체안에 아이디만 뽑아와야 할때 사용하자!
        User user1 = post.getUser(); // user객체에 있는 아이디만 뽑아와서 비교를 해야하니까 이렇게 써버렸다
        post.getUser().getId();
